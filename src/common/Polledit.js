...
const Polledit = (props) => {
    ...
    const genres = [
        ['소설', '시/에세이', '인문', '가정/육아', '요리', '건강', '취미/실용', '경제', '자기계발'],
        ['정치/사회', '역사/문화', '종교', '예술', '사회과학', '자연과학', '공학', '여행', '컴퓨터과학'],
        ...
        <div id="popup" className={`popup ${showPopup ? "show-popup" : ""}`}>
            ...
            <div className="content">
                ...
                <div className='positioned-container'>
                    <p className="error-message" style={{ display: errorMessage ? 'block' : 'none' }}>{errorMessage}</p>
                    <button className="signup-btn" onClick={submitForm}>수정</button>
                </div>
            </div>
        </div>
    );
